...
describe('Overview', () => {
  ...
  it('renders the hero, blurb and every overview section', () => {
    ...
    expect(screen.getByRole('heading', { level: 2, name: /jwt/i })).toBeInTheDocument();
    expect(screen.getByText(JWT.blurb)).toBeInTheDocument();
    // The install / quick-start / comparison / going-further / features sections.
    for (const suffix of ['install', 'quick', 'cmp', 'more', 'feat']) {
      expect(container.querySelector(`#${JWT.id}-${suffix}`)).not.toBeNull();
    }
    expect(screen.getByRole('heading', { name: 'Features' })).toBeInTheDocument();
    expect(container.querySelectorAll('ul.feat li').length).toBe(JWT.features.length);
  });
});
