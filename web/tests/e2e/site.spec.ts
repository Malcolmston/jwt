  ...
  for (const id of TAB_IDS) {
    ...
    for (let i = 0; i < count; i++) {
      ...
      if (/^https?:\/\//.test(href)) {
        ...
        // /jwt/api/. It is produced by the Go doc generator at deploy time,
        ...
        expect(resolved.pathname, `docs link should resolve under /jwt/`).toBe('/jwt/api/');
      } else {
...
test('docs tab renders the React API reference with a package listed', async ({ page }) => {
  ...
  // jwt is a single-package module (package jwt), so at least one package link is expected.
