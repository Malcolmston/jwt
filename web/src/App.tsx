...
export function App() {
  ...
  const brand = { title: 'malcolmston', sub: '/jwt', initials: 'jw', href: '#overview' };
  const footer: ReactNode = (
    <div>
      <span className="grad-text" style={{ fontWeight: 700 }}>malcolmston/jwt</span> — standard-library-only JSON Web Tokens for Go.
      <div className="small dim" style={{ marginTop: '.4rem' }}>MIT licensed · RFC 7519 (JWT) over RFC 7515 (JWS), zero dependencies</div>
    </div>
  );

  return (
    <Layout brand={brand} tabs={TABS} active={active} onNav={go} github="https://github.com/malcolmston/jwt" footer={footer}>
